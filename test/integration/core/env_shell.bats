...
@test "$DRIVER: test bash / zsh notation" {
  ...
  [[ ${lines[1]} == "export DOCKER_HOST=\"$(docker-machine url $NAME)\"" ]]
...
@test "$DRIVER: test powershell notation" {
  ...
  [[ ${lines[1]} == "$Env:DOCKER_HOST = \"$(docker-machine url $NAME)\"" ]]
...
@test "$DRIVER: test cmd.exe notation" {
  ...
  [[ ${lines[1]} == "set DOCKER_HOST=$(docker-machine url $NAME)" ]]
...
@test "$DRIVER: test fish notation" {
  ...
  [[ ${lines[1]} == "set -x DOCKER_HOST \"$(docker-machine url $NAME)\"" ]]
